...
export default function ProjectWorkersModal({ jobId, visible, onClose }: Props) {
  const [workers, setWorkers] = useState<ConnectionUser[]>([]);
  const [addOpen, setAddOpen] = useState(false);
  const [connections, setConnections] = useState<ConnectionUser[]>([]);

  const refresh = () => {
    if (jobId) listJobWorkers(jobId).then(setWorkers);
  };
  ...
  const handleDelete = (workerId: number) => {
    Alert.alert("Remove worker?", undefined, [
      { text: "Cancel", style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: async () => {
          await removeJobWorker(jobId, workerId);
  ...
  const handleAdd = async (userId: number) => {
    await addJobWorker(jobId, userId);
    setAddOpen(false);
    refresh();
  };
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <View style={styles.wrap}>
        <View style={styles.header}>
          ...
          <Pressable onPress={openAdd} style={{ padding: 8 }}>
        ...
        >
          <View style={styles.modalWrap}>
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <Text style={styles.empty}>No available connections.</Text>
              }
            />
          </View>
        </Modal>
