...
export default function PersonalInfo() {
  ...
  const profile =
    profiles[userId] ??
    defaultProfile(userId, user?.username ?? "You", (user?.role ?? "manager") as any);
  ...
  useEffect(() => {
    if (!profiles[userId]) {
      ensureProfile(userId, user?.username ?? "You", (user?.role ?? "manager") as any, token ?? undefined);
    }
  ...
  const save = async () => {
    ...
    upsertProfile(
      {
        ...(profile ?? defaultProfile(userId, username || "You", (user?.role ?? "manager") as any)),
        id: userId,
